...
`restore <file>`, `restore -`, `restore --list`, `--version`, `--help` and `<command> --help`,
names and flags matched exactly, case included.
...
- Stdout SHALL carry only data (the `backup -` backup, the `restore --list` listing, the
  `--version` and `--help` text), so `healthcheck`, `backup <file>`, `restore <file>` and
...
- **THEN** each exits 0 and creates no file; `--version` prints exactly `kohaku <version>\n` with empty stderr; the help names `serve`, `healthcheck`, `backup`, `restore` and `restore --list`
