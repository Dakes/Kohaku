...
The `read` class SHALL be the only rate-limit class of this change, with budgets fixed in
the binary (not configurable): 300 per minute per IPv4 /32 or IPv6 /64 and 2400 per minute
per IPv6 /48, shared across all hosts. It SHALL cover every GET and HEAD to a public route of
the main-host or project-host router (every route is public in this change), including their
404 fallbacks, and SHALL NOT cover `/static/` paths, internal endpoints or 421 unknown-host
