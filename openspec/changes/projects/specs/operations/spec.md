...
`admin reset-password --email <address>`, `project create <slug> --name <name>` with an
optional `--host <host>` after the name, `--version`, `--help` and `<command> --help`
(`admin unlock --help`, `admin reset-password --help` and `project create --help` for the
two-word commands), names and flags matched exactly, case included.
...
- Stdout SHALL carry only data (the `backup -` backup, the `restore --list` listing, the
  `admin reset-password` link, the `--version` and `--help` text), so `admin unlock`,
...
- **THEN** each exits 0 and creates no file; `--version` prints exactly `kohaku <version>\n` with empty stderr; the help names `serve`, `healthcheck`, `backup`, `restore`, `restore --list`, `admin unlock`, `admin reset-password` and `project create`
