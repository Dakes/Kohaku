...
`kohaku serve` (before opening the database, until it exits), `kohaku restore` (before
writing to `/data`, until it finishes) and `kohaku admin rekey` (before opening the database,
until it finishes) SHALL hold an exclusive lock on `/data/kohaku.lock`,
creating the file if missing. When the lock is held, each SHALL exit non-zero with an error
